...
public class Main{
    ...
    static void tambahKaryawan(Scanner input){
        ...
        while (!validInput) { 
            try {
                ...
                karyawanBaru.setGaji(pengalamanKaryawan);
                validInput = true;
            } catch (InputMismatchException e){
        ...
        if (karyawanBaru.getUmur() <= 18  | karyawanBaru.getPengalaman().getPendidikan().equals("SD") | karyawanBaru.getPengalaman().getPendidikan().equals("SMP")
         | karyawanBaru.getProjek().getLamaProjek() <= 4){ 
            playMusic(0); 
    ...
    static void playMusic(int confirm){ 
        if (confirm == 0){ 
            try {
                File musiclocation = new File("gagal.wav");
            ...
            try {
                File musiclocation = new File("lulus.wav");
